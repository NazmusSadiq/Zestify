...
export default function News_Media() {
  ...
  const loadLocalNews = async () => {
    ...
    try {
      ...
      let filtered = allNews.filter(
        article =>
          article.tags?.includes("movie") || article.tags?.includes("tv-series") || article.tags?.includes("tv-show") || article.tags?.includes("media")
      );
  ...
  return (
    <View style={styles.container}>
      {articles.length === 0 ? (
        <Text style={styles.title}>No movie or TV series news found.</Text>
      ) : (
        <FlatList
          data={articles}
